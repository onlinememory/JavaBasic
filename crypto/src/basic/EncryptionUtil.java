...
import static basic.ByteToHex.bytesArrayToHexString;
import static basic.ByteToHex.hexStringToBytesArray;
...
public class EncryptionUtil {
    ...
    public static String getHash(String source, String hashType) {
        ...
        try {
            ...
            byte[] bs = md.digest(source.getBytes(StandardCharsets.UTF_8));
            System.out.println(Arrays.toString(bs));
            String hexStr = bytesArrayToHexString(bs);
//            System.out.println();

            md.update(source.getBytes()); // 通过使用 update 方法处理数据,使指定的 byte数组更新摘要
            byte[] encryptStr = md.digest(); // 获得密文完成哈希计算,产生128 位的长整数
            System.out.println("Algorithm : " + md.getAlgorithm() + ",\t" + "Encrypt String length : " + encryptStr.length);
            char str[] = new char[encryptStr.length * 2]; // 每个字节用 16 进制表示的话，使用两个字符
            int k = 0; // 表示转换结果中对应的字符位置
            int c = 0;
            for (int i = 0; i < encryptStr.length; i++) { // 从第一个字节开始，对每一个字节,转换成 16 进制字符的转换
                byte byte0 = encryptStr[i]; // 取第 i 个字节

                str[k++] = hexDigits[byte0 >>> 4 & 0xf]; // 取字节中高 4 位的数字转换, >>> 为逻辑右移，将符号位一起右移
                str[k++] = hexDigits[byte0 & 0xf]; // 取字节中低 4 位的数字转换
//                System.out.println(c++ + "\t" + byte0 + " : " + (byte0 >>> 4 & 0xf) + "-" + (hexDigits[byte0 >>> 4 & 0xf]) + "\t" + (byte0 & 0xf) + "-" + (hexDigits[byte0 & 0xf]));
            }
            return new String(hexStr); // 换后的结果转换为字符串
    ...
    public static void main(String[] args) {
        hexStringToBytesArray("08E2A3810D8426443ECACAF47AEEDD17");
        hexStringToBytesArray("A");
        System.out.println(getHash("A", "MD2"));
//        System.out.println(getHash("A", "MD5"));
//        System.out.println(getHash("A", "SHA-256"));
//        System.out.println(getHash("A", "SHA-384"));
//        System.out.println(getHash("A", "SHA-512"));
